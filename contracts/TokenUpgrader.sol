...
contract TokenUpgrader is Ownable {
    ...
    */
    function upgradeEdit() external {
        ...
        // Create a listing of valid IDs (vesting schedules launched before round 3)
        uint256[] memory validIds = new uint256[](infos.length);
        uint256 j = 0;
        for (uint256 i = 0; i < infos.length; i++) {
            if (infos[i].startTime <= vestingDeadline) {
                validIds[j] = infos[i].id;
                j++;
            }
        }

        // Compare the valid scheduleIDs in the claimData with validIds and calculate validClaimedAmount
        uint256 validClaimedAmount = 0;
        for (uint256 i = 0; i < claimInfos.length; i++) {
            for (uint256 k = 0; k < j; k++) {
                if (claimInfos[i].scheduleID == validIds[k]) {
                    // Add amounts to validClaimedAmount
                    validClaimedAmount = Math.add(
                        validClaimedAmount,
                        claimInfos[i].claimedAmount
                    );
                }
            }
        }

        // Subtract tokens that we already upgraded for this user, which prevents users claiming more tokens than they are owed
        uint256 toUpgrade = validClaimedAmount.sub(
